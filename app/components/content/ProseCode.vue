<template>
  <div class="relative group">
    <div v-if="filename" class="absolute top-0 right-0 px-3 py-1 text-xs font-mono text-slate-300 bg-slate-800 rounded-bl-lg rounded-tr-lg border-l border-b border-slate-700 opacity-0 group-hover:opacity-100 transition-opacity">
      {{ filename }}
    </div>
    <button
      @click="copy"
      class="absolute top-2 right-2 p-1.5 rounded-md bg-slate-800 border border-slate-700 text-slate-300 hover:text-white hover:bg-slate-700 opacity-0 group-hover:opacity-100 transition-all z-10"
